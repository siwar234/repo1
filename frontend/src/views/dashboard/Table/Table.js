...
import { getListTicketsByproject, updateTicketPosition, updatingtickets } from '../../../JS/actions/Tickets'
...
const renderTicketBlock = ( ticket ) => {
  ...
  />
            ...
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: "20px", marginLeft: '15px' }}>
              ...
              <img 
              src={ ticket?.Types?.TypesTitle === 'Bug'
                ? image1
    ...
    return (
        <PageContainer title="Dashboard" description="This is Table">
          ...
          <Box display="flex"  flexDirection={"row"} >
            ...
            <Box mb={2}>
            <FiltragePerPerson selectedPerson={selectedPerson} setSelectedPerson={setSelectedPerson} handlePersonClick={handlePersonClick}  projectId={projectId} checked={checked} setchecked={setchecked}
              /></Box>
</Box>
      ...
      {Array.isArray(workflows) && workflows.map((workflow) => (
          <Box style={{ marginRight: '30px', minWidth: '280px' }} key={workflow._id}>
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', marginLeft: '20px', marginTop: '20px' }}>
                <div style={{
                  ...
                  height: '16px',
                  borderRadius: '50%',
                  backgroundColor:     workflow.workflowTitle === 'TO DO'
                  ? '#7ca1f35e'
                ...
                >
                  ...
                  <span style={{ marginLeft: '5px' }}>
                    ...
                    {allTickets.filter(ticket => ticket.workflow.workflowTitle === workflow.workflowTitle).length}
                  </span>
                </Typography>
              </div>
            ...
            <Droppable droppableId={workflow._id}>
                  {(provided) => (
                    <Box {...provided.droppableProps} ref={provided.innerRef} sx={{ minHeight: 500 }}>
                      {allTickets
                        .filter((ticket) => ticket.workflow?._id === workflow._id && 
                          ...
                          (selectedPerson.length === 0 || 
                            (selectedPerson.includes('Non-Assigned') && !ticket.ResponsibleTicket) ||
